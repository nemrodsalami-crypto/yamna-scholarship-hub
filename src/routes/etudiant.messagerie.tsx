...
export const Route = createFileRoute("/etudiant/messagerie")({
  head: () => ({ meta: [{ title: "Messagerie — YAM'NA" }] }),
  component: EtudiantMessagerie,
});

const THREADS = MSG_THREADS.slice(0, 4);

function EtudiantMessagerie() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Messagerie"
        description="Échanges avec votre référent COMILOG et le service des bourses."
      />
      ...
      <div className="flex h-[calc(100vh-260px)] min-h-[480px] gap-4">
        ...
        <Card className="flex w-72 shrink-0 flex-col overflow-hidden p-0">
          <div className="border-b px-4 py-3">
            <p className="text-sm font-semibold">Mes messages</p>
            <Badge variant="secondary" className="mt-1 h-5 rounded-md px-1.5 text-[10px]">2 non lus</Badge>
          </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active && "bg-secondary/60 border-l-2 border-primary"
                    )}
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {unread && <span className="absolute right-3 top-3 h-2 w-2 rounded-full bg-primary" />}
                    </div>
                  </button>
                );
              })}
