...
import { User, GraduationCap, Wallet, MessagesSquare, LogOut, ChevronRight, Camera, Menu, X, Bell } from "lucide-react";
...
const ME = ETUDIANTS[0]; // Nadia Ondo

const NAV = [
  { label: "Profil", url: "/etudiant/profil", icon: User },
  { label: "Cursus", url: "/etudiant/cursus", icon: GraduationCap },
  { label: "Suivi bourse", url: "/etudiant/bourse", icon: Wallet },
  { label: "Messagerie", url: "/etudiant/messagerie", icon: MessagesSquare, badge: "2" },
];
...
function EtudiantLayout() {
  ...
  const ProfileCard = (
    <div className="border-b bg-gradient-to-b from-primary/6 to-transparent px-4 pb-5 pt-4">
      ...
      <div className="relative mx-auto w-fit">
        ...
        >
          <Avatar className="h-20 w-20 ring-2 ring-background shadow-md">
            <AvatarFallback className={cn("bg-gradient-to-br text-2xl font-bold text-white", ME.avatarTone)}>
              {ME.initials}
            </AvatarFallback>
          </Avatar>
          {/* Overlay upload */}
          ...
          <button className="absolute -bottom-1 -right-1 grid h-6 w-6 place-items-center rounded-full bg-primary text-white ring-2 ring-background shadow">
      ...
      <div className="mt-3 text-center">
        ...
        <p className="text-[11px] text-muted-foreground">{ME.matricule}</p>
        <p className="mt-0.5 text-[11px] text-muted-foreground">{ME.niveau} · {ME.school}</p>
        <Badge className="mt-2 h-5 border-success/30 bg-success/12 px-2 text-[10px] text-success hover:bg-success/15">
          Boursière {ME.bourseType}
        </Badge>
      </div>
    </div>
  );
    ...
    <div className="flex min-h-screen w-full bg-background">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b bg-background/90 px-4 backdrop-blur-md">
          ...
          {/* Mini profile card in topbar */}
          <div className="flex items-center gap-2.5 rounded-xl border bg-card px-3 py-1.5 shadow-sm">
            <Avatar className="h-7 w-7 shrink-0">
              <AvatarFallback className={cn("bg-gradient-to-br text-[10px] font-bold text-white", ME.avatarTone)}>{ME.initials}</AvatarFallback>
            </Avatar>
            <div className="hidden sm:block">
              <p className="text-xs font-semibold leading-none">{ME.name}</p>
              <p className="mt-0.5 text-[10px] text-muted-foreground">{ME.matricule} · {ME.niveau}</p>
            </div>
            <div className="ml-1 hidden items-center gap-1 rounded-full border border-success/30 bg-success/10 px-1.5 py-0.5 text-[10px] font-semibold text-success sm:flex">
              <span className="h-1.5 w-1.5 rounded-full bg-success" /> Boursière
            </div>
          </div>
